...
- Meal photos: **not** persisted server-side; full base64 stored in browser localStorage (frontend), stripped from the persisted copy above 24 kB to protect quota (#66). Backend deletion cannot reach that localStorage copy (documented gap).
...
- Secrets don't leak to responses/logs; only `print(f"Error: {e}")` on the generic path (no user content). CORS `*` + credentials is dev-only and documented.
