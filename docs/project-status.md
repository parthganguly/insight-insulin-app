...
**Next decision:** explicitly approve or reject the bounded correction proposed in
[issue #134](https://github.com/parthganguly/insight-insulin-app/issues/134).
The implementation defect is now reproduced on the owner's local J9 source;
no scoring edit has been approved or made yet.

**Checkpoint:** 2026-09-12. Remote source reviewed:
`56e218be6ef3386de473754e9f49ac4680ca7d45`. Issue #134 was subsequently
reproduced on owner-local `codex/j9-hardening-final` at
`f1db1b7798927bb75a5378f08543e23a4e34385a` using isolated synthetic
`POST /meals/preview` requests. This is a dated status/navigation document,
not a new architecture decision, scientific validation or release approval.
...
## Later local work: reported versus reproduced
...
used for the earlier remote audit, so its full J9 implementation is still not
freshly certified here. Do not reset or overwrite it to match remote main.

Issue #134 **was** subsequently reproduced directly on that exact local source:
59 isolated synthetic preview requests showed that the macro-fallback load did
not scale with `quantity`, while equivalent provided-FII and exact-FII controls
did. For example, two ordinary servings and one combined serving had identical
400 kcal / 60 g carbohydrate / 20 g protein / 10 g fat totals, but the rough
fallback returned load 21.21 versus 42.42 and acute score 70.7 versus 141.4.
At quantity zero the fallback could still return a positive load. No database,
provider, real meal, branch change or source edit was involved. This confirms
an implementation/input-contract defect; it does not validate or invalidate
the fallback coefficients themselves.
...
but did not repair recognition. These are reported results, not integrations
in this documentation change. No replacement model is selected here. The five
repeatedly used photos are development cases, not untouched generalization
evidence.

## What still needs proof

1. **Corrected input consistency.** The defect is confirmed; the proposed fix
   still needs explicit approval, versioning treatment, Python/Rust tests,
   before/after output evidence and independent review before it can merge.
2. **Scientific evidence.** Dataset provenance, partial decomposition coverage,
   ...
   questions. Fixing #134 makes the implementation coherent; it does not make
   the fallback scientifically validated.
...
**Now:** owner approves or rejects the bounded #134 contract correction. If
approved, implement it on an isolated branch/worktree with equivalence tests,
Python/Rust parity, formula/version review and independent diff review.
Historical saved results remain unchanged unless a separate migration is
explicitly approved.

**Then:** execute the existing provenanced-data workplan. Dataset research can
proceed independently of camera-model selection.
...
Do not change coefficients, thresholds, mappings, historical results or
protected copy as part of #134 unless separately approved. Keep failed
experiments and old decisions; remove stale instructions from the active
reading path rather than deleting the record of why the project changed
direction. This page is not authorization for another provider experiment,
